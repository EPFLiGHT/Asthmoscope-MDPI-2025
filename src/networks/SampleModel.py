...
from src.networks.Cnn10Att import Cnn10Att
...
class SampleModel(nn.Module):
    def __init__(self, feature_model, n_feats=64, conv_channels=32, pos_emb_dim=0, fc_features=64,
        ...
        # Only "dense" model is used in this experiment
        if feature_model != "dense":
            raise ValueError(f"Only 'dense' feature_model is supported, got: {feature_model}")
        
        self.features = Cnn10Att(classes_num=classes_num)

        # Note: classifier is not used for dense model
    ...
    def preprocess(self, x, mixup_lambda):
    ...
    def forward(self, x, mixup_lambda=None):
        ...
        x = self.preprocess(x, mixup_lambda)

        # Only "dense" model is supported
        out = self.features(x)
        output_dict = {
